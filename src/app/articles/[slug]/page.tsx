...
function formatQuoteCitations(content: string) {
    return content.replace(
        /(<blockquote\b[^>]*>[\s\S]*?)<p>\s*(?:—|&mdash;)\s*([^<]+)<\/p>([\s\S]*?<\/blockquote>)/gi,
        "$1<cite>$2</cite>$3",
    );
}
...
export default async function ArticlePage({params}: { params: Promise<{ slug: string }> }) {
    ...
    const articleContent = `max-w-none py-9 text-base leading-[1.65] text-[#343943]
        ...
        [&_blockquote]:my-12 [&_blockquote]:border-l-4 [&_blockquote]:border-[var(--blue)] [&_blockquote]:bg-[#edf6ff] [&_blockquote]:px-7 [&_blockquote]:py-7 [&_blockquote]:font-[Hanken] [&_blockquote]:text-[18px] [&_blockquote]:font-bold [&_blockquote]:italic [&_blockquote]:leading-[1.55] [&_blockquote]:text-[var(--ink)] [&_blockquote_svg]:mb-4 [&_blockquote_svg]:h-7 [&_blockquote_svg]:w-8 [&_blockquote>p]:m-0 [&_blockquote_cite]:mt-5 [&_blockquote_cite]:block [&_blockquote_cite]:font-[Inter] [&_blockquote_cite]:text-xs [&_blockquote_cite]:font-semibold [&_blockquote_cite]:not-italic [&_blockquote_cite]:text-[var(--blue)]
                ...
                <div className="min-w-0">
                    <header className="text-left">
                        <div className="flex items-center gap-4 text-xs">
                            <span
                                className="bg-[#dcecff] px-2.25 py-1.25 text-sm font-extrabold text-(--blue)">{article.resourceType === "temoignage" ? "TÉMOIGNAGE" : article.resourceType === "video" ? "VIDÉO" : "ARTICLE"}</span>{article.publishedDate &&
                ...
                <aside
                    ...
                    <div className={asideCtaCard}>
                        ...
                        <div className="relative">
                            <h2 className="m-0 font-[Hanken] text-2xl font-bold leading-[1.2] text-[var(--ink)]">{siteConfig?.articleSidebarPrimaryTitle || "Prêt pour la révolution ?"}</h2>
                            <p className="mb-6 mt-3 text-base leading-[1.55] text-[var(--muted)]">{siteConfig?.articleSidebarPrimaryText || "Découvrez comment nos solutions ERP intègrent l’IA pour votre métier."}</p>
                            <CmsButton className="!min-h-11 !px-4 !text-base" button={siteConfig?.articleSidebarPrimaryButton || {
                    ...
                    {related.length > 0 && <div className={asideCard}>
                        <h2 className="mb-4 mt-0 font-[Hanken] text-2xl font-bold text-[var(--blue)]">À lire aussi</h2>
                    ...
                    <div className="relative overflow-hidden rounded-[6px] bg-[var(--blue)] p-6 text-white shadow-[0_8px_20px_rgba(18,50,85,.08)]">
                        ...
                        <div className="relative"><h2 className="m-0 font-[Hanken] text-2xl font-bold leading-[1.2]">{siteConfig?.articleSidebarSecondaryTitle || "Prêt pour la révolution ?"}</h2><p className="mb-6 mt-3 text-base leading-[1.55] text-[#d7e3ff]">{siteConfig?.articleSidebarSecondaryText || "Découvrez comment nos solutions ERP intègrent l’IA pour votre métier."}</p><CmsButton className="!min-h-11 !px-4 !text-base" button={siteConfig?.articleSidebarSecondaryButton || {label: "Voir nos éditeurs", href: "/#solutions", style: "light"}}/></div>
                    </div>
                </aside>
            </div>
        </div>
