...
export default async function ArticlePage({params}: { params: Promise<{ slug: string }> }) {
    ...
    return <article className="bg-[var(--sky)] pb-24 pt-[42px]">
        <div className="mx-auto w-[calc(100%-48px)] max-w-[1192px] max-[800px]:w-[calc(100%-32px)] max-[800px]:max-w-[680px]">
            <nav className="mb-[38px] flex items-center gap-2 text-xs text-[var(--muted)]" aria-label="Fil d’Ariane"><Link href="/">Accueil</Link><span>›</span><Link
                href="/articles">Ressources</Link><span>›</span><strong>{article.title}</strong></nav>
            <div className="grid grid-cols-[minmax(0,2.25fr)_minmax(270px,1fr)] items-start gap-8 max-[1000px]:grid-cols-1">
                <div className="min-w-0">
                    <header className="text-left">
                        <div className="flex items-center gap-4 text-xs">
                            <span className="bg-[#dcecff] px-[9px] py-[5px] font-extrabold text-[var(--blue)]">{article.resourceType === "temoignage" ? "TÉMOIGNAGE" : article.resourceType === "video" ? "VIDÉO" : "ARTICLE"}</span>{article.publishedDate &&
                            <time>{new Intl.DateTimeFormat("fr-FR", {dateStyle: "medium"}).format(new Date(article.publishedDate))}</time>}
                        </div>
                        <h1 className="mb-[22px] mt-[18px] font-[Hanken] text-[44px] font-extrabold leading-[1.16] text-[var(--blue)] max-[800px]:text-[38px]">{article.title}</h1>{article.readingTime &&
                        <p className="flex items-center gap-[7px] text-[13px] text-[var(--muted)]"><Clock3 size={16}/> {article.readingTime} min de lecture</p>}
                    </header>
                    {cover &&
                        <div className="relative mb-[35px] mt-7"><Image className="w-full rounded-lg shadow-[0_15px_35px_rgba(18,50,85,.12)]" src={cover} alt={article.cover?.alternativeText || ""}
                ...
                <aside className="sticky top-[120px] grid gap-[18px] max-[1000px]:static max-[1000px]:grid-cols-3 max-[800px]:grid-cols-1">
                    ...
                    {related.length > 0 && <div className={asideCard}><h2>À lire aussi</h2>{related.map((item) => <Link
                        className="flex gap-3 border-t border-[var(--line)] py-3 text-[13px] font-semibold" href={`/articles/${item.slug}`} key={item.id}>{item.cover?.url &&
                        <Image className="object-cover" src={mediaUrl(item.cover.url)} alt="" width={72}
                               height={58}/>}<span className="flex flex-col gap-[5px]"><small className="text-[10px] text-[var(--blue)]">{item.resourceType === "temoignage" ? "TÉMOIGNAGE" : "ARTICLE"}</small>{item.title}</span></Link>)}
                    </div>}
                    <div className={`${asideCtaCard} !bg-[var(--blue)] !text-white [&_p]:!text-[#d7e3ff]`}><h2>Prêt pour la révolution ?</h2><p>Découvrez comment
