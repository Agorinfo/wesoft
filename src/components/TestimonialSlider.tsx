...
export function TestimonialSlider({ testimonials, metrics, quoteIcon }: { testimonials: Testimonial[]; metrics: Metric[]; quoteIcon?: CmsIconValue }) {
  ...
  const hasMetrics = slideMetrics.length > 0;
  ...
  const sliderGridClass = hasMetrics
    ? "grid-cols-[25px_minmax(0,1fr)_minmax(0,1fr)_25px]"
  ...
  return <section className="bg-[var(--blue)] py-20 text-white max-[800px]:py-16" aria-roledescription="carrousel" aria-label="Témoignages" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocus={() => setPaused(true)} onBlur={() => setPaused(false)}>
    <div className={`${CONTENT} grid ${sliderGridClass} items-center gap-16 max-[1000px]:grid-cols-[25px_minmax(0,1fr)_25px] max-[1000px]:gap-8 max-[800px]:gap-4 max-[600px]:grid-cols-[20px_minmax(0,1fr)_20px]`}>
      ...
      {hasMetrics && <div className={`grid h-40 grid-cols-2 gap-8 border-l border-white/20 pl-[65px] max-[1000px]:col-start-2 max-[1000px]:row-start-2 max-[1000px]:border-l-0 max-[1000px]:border-t max-[1000px]:px-0 max-[1000px]:pt-8 max-[800px]:h-auto max-[800px]:gap-x-4 max-[800px]:gap-y-6 ${slideMetrics.length <= 2 ? "content-center" : ""}`}>
