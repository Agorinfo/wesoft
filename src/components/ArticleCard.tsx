...
export function ArticleCard({ article }: { article: Article }) {
  ...
  return <article className="flex h-full flex-col overflow-hidden border-2 border-[var(--line)] bg-white shadow-[0_5px_15px_rgba(18,50,85,.06)] transition hover:-translate-y-[5px] hover:shadow-[var(--shadow)]">
    ...
    <div className="flex flex-1 flex-col p-[18px]">
      ...
      <Link className="mt-auto flex min-h-[46px] items-center justify-center border-2 border-[var(--blue)] px-4 text-base font-semibold text-[var(--blue)]" href={`/articles/${article.slug}`}>{article.buttonLabel || (article.resourceType === "temoignage" ? "Lire le témoignage" : "Lire l’article")}</Link>
    </div>
  </article>;
}
