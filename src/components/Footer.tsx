...
export function Footer({config}: { config: SiteConfig }) {
    ...
    const columnClass = "flex min-w-0 flex-col gap-3 [&_a]:text-base [&_a]:text-[var(--muted)] [&_a]:hover:text-[var(--blue)]";
    return <footer className="border-t border-(--line) pb-7.5 pt-20">
        <div className={`mx-auto grid w-[calc(100%-48px)] max-w-298 grid-cols-1 gap-9.5 sm:grid-cols-2 md:grid-cols-3 ${desktopGrid} max-[800px]:w-[calc(100%-32px)] max-[800px]:max-w-170`}>
            ...
            {columns.map((column) => <div className={columnClass} key={column.title}>
                <h3 className="mb-2.5 mt-1 text-sm font-semibold uppercase tracking-[.08em]">{column.title}</h3>
                {column.links?.map((link) => <Link href={link.href}
                                                   key={`${link.label}-${link.href}`}>{link.label}</Link>)}
            </div>)}
            <div className={columnClass}><h3 className="mb-2.5 mt-1 text-sm font-semibold uppercase tracking-[.08em]">{config.footerCtaTitle || "Un projet ?"}</h3><CmsButton
                button={config.headerButton} className="whitespace-nowrap px-4"/></div>
        </div>
        <div className="mx-auto mt-15 flex w-[calc(100%-48px)] max-w-298 justify-between border-t border-(--line) pt-6.25 text-base text-(--muted) max-[600px]:flex-col max-[600px]:gap-4 max-[800px]:w-[calc(100%-32px)] max-[800px]:max-w-170"><span>{config.copyright}</span>
            <div className="flex gap-4.5 max-[600px]:flex-wrap">{config.legalLinks?.map((link) => <Link className="text-base hover:text-(--blue)" key={link.href} href={link.href}>{link.label}</Link>)}</div>
        </div>
    </footer>;
}
