...
export function DynamicForm({ form, variant = "card" }: DynamicFormProps) {
  ...
  const inputClass = "min-h-12 w-full rounded-[5px] border border-[#cbd7e3] bg-[#fbfdff] px-[14px] py-[11px] text-[15px] text-[var(--ink)] outline-none transition placeholder:text-[#8994a2] hover:border-[#aebfd0] focus:border-[var(--blue)] focus:bg-white focus:ring-2 focus:ring-[#b7d8f7]";
      ...
      {form.fields?.map((field) => (
        <div className={`flex flex-col gap-2 ${field.width !== "half" ? "col-span-full" : "max-[600px]:col-span-full"}`} key={field.name}>
          {field.type === "checkbox" ? (
            ...
            <>
              <label className="text-sm font-bold text-[var(--ink)]" htmlFor={field.name}>{field.label}{field.required ? <span className="text-[var(--blue)]"> *</span> : null}</label>
