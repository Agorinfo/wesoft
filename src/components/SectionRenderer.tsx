...
async function renderSection(block: PageBlock, key: string, hasHero: boolean, hasBreadcrumb: boolean, removeHeroBottomPadding: boolean) {
  switch (block.__component) {
    case "sections.hero": return <HeroSection key={key} block={block} hasBreadcrumb={hasBreadcrumb} removeBottomPadding={removeHeroBottomPadding} />;
...
export async function SectionRenderer({ blocks = [], hasBreadcrumb = false, removeHeroBottomPadding = false }: { blocks?: PageBlock[]; hasBreadcrumb?: boolean; removeHeroBottomPadding?: boolean }) {
  const hasHero = blocks.some((block) => block.__component === "sections.hero");
  return <>{await Promise.all(blocks.map((block, index) => renderSection(block, `${block.__component}-${block.id}-${index}`, hasHero, hasBreadcrumb, removeHeroBottomPadding)))}</>;
}
