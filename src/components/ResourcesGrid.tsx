...
export function ResourcesGrid({ articles }: { articles: Article[] }) {
  ...
  return <>
    <div className="mb-7 flex items-center justify-between gap-6 max-[600px]:flex-col max-[600px]:items-stretch">
      <div className="flex gap-2 max-[600px]:overflow-x-auto" aria-label="Filtrer les ressources">{filters.map(([value, label]) => <button key={value} type="button" className={`rounded border px-[18px] py-[9px] font-semibold ${filter === value ? "border-[var(--blue)] bg-[var(--blue)] text-white" : "border-[var(--line)] bg-white text-[var(--muted)]"}`} onClick={() => setFilter(value)}>{label}</button>)}</div>
      <label className="flex h-10 items-center gap-2 border border-[var(--line)] bg-white px-3"><Search size={17} aria-hidden /><span className="sr-only">Rechercher</span><input className="min-w-[220px] border-0 outline-none max-[600px]:w-full max-[600px]:min-w-0" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher un article…" /></label>
    </div>
