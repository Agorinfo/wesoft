...
export function CmsButton({ button, className = "" }: { button?: ButtonData | null; className?: string }) {
  ...
  const classes = `inline-flex min-h-[50px] items-center justify-center rounded border-2 border-transparent px-[31px] text-[15px] font-bold tracking-[.02em] transition duration-200 ease-out hover:-translate-y-0.5 motion-reduce:transform-none motion-reduce:transition-none ${variants[button.style || "primary"]} ${className}`;
