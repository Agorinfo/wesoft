...
const variants = {
  ...
  secondary: "!border-2 !border-[var(--blue)] !bg-transparent !text-[var(--blue)]",
...
export function CmsButton({ button, className = "" }: { button?: ButtonData | null; className?: string }) {
  ...
  const classes = `inline-flex min-h-[50px] items-center justify-center rounded-[4px] border-0 px-[31px] text-base font-semibold tracking-[.02em] transition duration-200 ease-out hover:-translate-y-0.5 motion-reduce:transform-none motion-reduce:transition-none ${variants[button.style || "primary"]} ${className}`;
