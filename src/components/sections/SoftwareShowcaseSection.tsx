    ...
    <div className={CONTENT}>
      ...
      <div className="grid gap-6">
        {list<Software>(block.items).map((item, index) => {
          ...
          return <article className="grid min-h-70.5 grid-cols-[360px_1fr] border border-(--line) border-t-4 bg-[#fbfbff] max-[1000px]:grid-cols-[280px_1fr] max-[800px]:grid-cols-1" style={{ borderTopColor: accent }} key={item.id || item.name || index}>
            ...
            <div className="px-6 py-5.75">
              <div className="flex h-18 items-start justify-between gap-6 max-[600px]:h-auto max-[600px]:flex-col">
                ...
                <CmsButton className="min-w-46.25" button={item.button} />
              </div>
              <div className="grid grid-cols-2 gap-8.5 pt-4 max-[800px]:grid-cols-1 max-[600px]:pt-7.5">
                <div><h3 className="mb-2 font-[Hanken] text-2xl font-bold" style={{ color: accent }}>{item.sector}</h3><p className="m-0 text-sm leading-[1.55] text-(--muted)">{item.description}</p></div>
                <ul className="m-0 list-none p-0">{item.capabilities?.map((capability) => <li className="border-b border-(--line) pb-3 pt-2 font-[Hanken] text-sm font-semibold leading-[1.4]" style={{ color: accent }} key={capability.id || capability.text}>{capability.text}</li>)}</ul>
              </div>
            </div>
          </article>;
        })}
