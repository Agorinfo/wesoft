...
export function LegalContentSection({ block, showIntro = true }: { block: PageBlock; showIntro?: boolean }) {
  const sections = list<LegalSection>(block.sections);
  return <Fragment>
    {showIntro && <section className="bg-white py-20"><div className={CONTENT}>
    ...
    <section className={`pb-20 pt-20 ${backgroundClass(block)}`}><div className={`${CONTENT} grid grid-cols-[268px_1fr] items-start gap-9 max-[1000px]:grid-cols-[220px_1fr] max-[800px]:grid-cols-1`}>
      <aside className="sticky top-[125px] max-[800px]:static"><strong className="text-sm tracking-[.06em]">SOMMAIRE</strong><LegalTableOfContents sections={sections} /></aside>
      <div className="grid gap-6">{sections.map((item) => <article id={item.anchorId} className="scroll-mt-[120px] rounded-md border-2 border-[var(--line)] bg-white p-6 max-[600px]:p-5" key={item.anchorId}><span className="mb-5 block h-1 w-12 bg-[var(--blue)]" /><h2 className="mb-[22px] font-[Hanken] text-[32px] font-bold leading-[1.2] max-[600px]:text-[28px]">{item.title}</h2><div className="text-base leading-[1.65] text-[#3f4650] [&_p]:mb-2.5 [&_p]:mt-0 [&_ul]:my-5 [&_ul]:list-none [&_ul]:space-y-3 [&_ul]:p-0 [&_ul>li]:relative [&_ul>li]:pl-6 [&_ul>li]:before:absolute [&_ul>li]:before:left-0 [&_ul>li]:before:top-[.6em] [&_ul>li]:before:h-2 [&_ul>li]:before:w-2 [&_ul>li]:before:rounded-full [&_ul>li]:before:bg-[var(--blue)]"><TiptapContent content={item.content} /></div></article>)}</div>
    </div></section>
  </Fragment>;
}
