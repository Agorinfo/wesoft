...
export function LegalContentSection({ block, showIntro = true }: { block: PageBlock; showIntro?: boolean }) {
  ...
  return <Fragment>
    <section className="bg-white py-20"><div className={CONTENT}>
      {showIntro && <><nav className="mb-5 flex items-center gap-2 text-xs text-[var(--muted)]"><Link href="/">Accueil</Link><span>›</span><strong className="overflow-hidden text-ellipsis whitespace-nowrap font-semibold text-[var(--blue)]">{text(block.title)}</strong></nav>
