...
function Card({item, index}: { item: Feature; index: number }) {
    ...
    return <article
        className="relative col-span-(--card-span) flex min-h-52.5 flex-col justify-center overflow-hidden rounded-lg px-8.5 py-9 max-[800px]:col-span-1 max-[600px]:min-h-47.5"
        style={style}>
        {item.icon
    ...
    return <section id={text(block.anchorId)} className={sectionClass(block)}>
        <div className={CONTENT}>
            <div className="flex items-center justify-between gap-8.75 max-[800px]:block">
                <SectionHeading className="mb-10.5 max-[800px]:mb-6" eyebrow={text(block.eyebrow)}
                                title={text(block.title)} body={text(block.text)}/>
            ...
            <div className="grid grid-cols-8 gap-4.5 max-[800px]:grid-cols-1">
                {list<Feature>(block.cards).map((item, index) => <Card item={item} index={index}
                                                                       key={item.id || index}/>)}
            </div>
        </div>
    </section>;
